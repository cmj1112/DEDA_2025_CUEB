...
# Load the text file
file_path = r"C:\Work\DEDA\pythonProject\Abstracts\Abstract_all.txt"
...
img=plt.imshow(zz, cmap='rainbow', interpolation='nearest')

# 添加彩色图例
cbar = plt.colorbar(img)
cbar.set_label('Word Probability', rotation=270, labelpad=20)

# Annotate the heatmap with words
for val, key in enumerate(DC.keys()):
    plt.text(-4.5, val + 0, key, horizontalalignment='center', verticalalignment='center')

# Set title and remove y-ticks (since we'll annotate manually)
plt.title("Heatmap of Topic-Word Probabilities",fontsize=16)
plt.yticks([])
